...
def publishSurfaces():
    if fileFormatCheck("surfacing"):
        ...
        cmds.confirmDialog( title='Warning', message="This is not a surfacing file. Please open a surfacing file to publish surfacers.")
...
def newVersFormat(obj, type):
    if obj == "":
        ...
        if (type == 'json'):
            vers = int(obj.split('_')[2].split('.')[0].split('v')[1])+1
        else:
            vers = int(obj.split('.')[1].split('v')[1])+1
        #verions
    if vers < 10:
        return "00" + str(vers)
    elif vers < 100:
...
def checkPrevFileAndSave(list):
    ...
    if (os.path.exists(pubFolderpath+'material/') == False):
        os.makedirs(pubFolderpath+'material/')
    if (os.path.exists(pubFolderpath+'source/') == False):
        os.makedirs(pubFolderpath+'source/')
    if (os.path.exists(pubFolderpath+'textures/') == False):
        os.makedirs(pubFolderpath+'textures/') 
    ...
    if 'fileName' not in locals():
        fileName = shaderFile = surfaceFile = ""
...
def setSurfaces():
    if fileFormatCheck("light"):
        ...
        if shaderVers is None:
            cmds.confirmDialog( title='Warning', message="No Object and Shaders selected.")
        else:
            theNodes = cmds.ls(dag = True, s = True, o = True)
            sceneGeo = ""
            #Go through scene
